...
var packageManager = function packageManager(state) {

  state.packages = {};
  var expiry = 1000 * 60 * 60 * 24; // 24 hours in ms

  return {

    addPackage: function addPackage(name) {
      state.packages[name] = state.packages[name] || [];
      var date = new Date(Date.now() + expiry);
      state.packages[name].push(date);
      return true;
    },

    activePackages: function activePackages() {
      var ret = {};
      var keys = Object.keys(state.packages);

      keys.forEach(function (k) {
        var last = state.packages[k].slice(-1)[0];
...
var User = function User(name) {
  ...
  return _extends({}, accessor(state), queueManager(state), packageManager(state));
};
